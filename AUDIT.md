...
| V18 | low | Hover still repaints the HEAD layer when emphasis changes (one full pass, ~15 ms for 480 k splats on software GL). | superseded — hover no longer restyles anything; the room dims only for a selected object. The detection bracket is the hit box: it hugs the object's own splats (a 1-in-3 sample kept at load, ≤ 4 k projected per frame) padded by 1.5 voxels at the object's depth, and anything inside it hovers and clicks, nearest object first where brackets overlap |
